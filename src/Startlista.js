...
const START_TIMES = {
  'H50': moment('2017-05-20 19:00'),
  'H21': moment('2017-05-20 19:00'),
  'H16': moment('2017-05-20 19:10'),
  'H14': moment('2017-05-20 19:20'),
  'H12': moment('2017-05-20 19:30'),
  'H10': moment('2017-05-20 19:40'),
  'D50': moment('2017-05-20 19:20'),
  'D70': moment('2017-05-20 19:10'),
  'D21': moment('2017-05-20 19:45'),
  'H70': moment('2017-05-20 19:40'),
  'D16': moment('2017-05-20 19:50'),
...
const Person = ({name, results, score, index, klass}) => {
  const time = START_TIMES[klass].clone()
  const startTime = time.add(index, 'minutes')
  return(
    <tr>
...
const KlassStartlista = ({klass, personer}) => {
  if (klass === '�M1' || klass === '�M4' || klass === '�M8' || klass === 'ÖM1' || klass === 'ÖM4' || klass === 'ÖM8') {
    return(
      null
    )
  }
      ...
      <table>
        <thead>
          <tr>
            ...
            <th style={{padding: '0 10px'}}>{'Totalpoäng'}</th>
            <th style={{padding: '0 10px'}}>{'Starttid'}</th>
          </tr>
        </thead>
        <tbody>
        { Object.keys(personer).map((hash, index) => {
          const person = personer[hash]
          return ( <Person key={generateKey()} index={index} klass={klass} {...person} />)
        })}
        </tbody>
      </table>
    </div>
...
class Startlista extends Component {
  ...
  render() {
    ...
    return (
      <div className="Startlista" style={{padding: '2em'}}>
        { Object.keys(total).length > 0 ? Object.keys(total).map((key) => {
          const personer = total[key]
          return (<KlassStartlista key={generateKey()} klass={key} personer={personer} />)
        }) : null }
        <h1 style={{paddingTop: '1em', textAlign: 'left'}}>Öppna klasser har fri starttid</h1>
      </div>
    );
  }
}

export default Startlista;
